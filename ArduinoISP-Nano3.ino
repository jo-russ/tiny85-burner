...
// in case you used common anode leds or soldered leds wrong way around
#define LED_INVERT
...
void loop(void) {
  // is pmode active?
  if (pmode) {
    digitalWrite(LED_PMODE, LED_HIGH);
  } else {
    digitalWrite(LED_PMODE, LED_LOW);
  }
  // is there an error?
  if (error) {
    digitalWrite(LED_ERR, LED_HIGH);
  } else {
    digitalWrite(LED_ERR, LED_LOW);
  }
...
void pulse(int pin, int times) {
  do {
    digitalWrite(pin, LED_HIGH);
    delay(PTIME);
    digitalWrite(pin, LED_LOW);
    delay(PTIME);
  } while (times--);
}
